...
module.exports = function apiMiddleware (app) {
  ...
  app.get('/api/product/:id', (req, res) => {
    ...
    if (!product) return res.status(404).json({ error: { message: 'Product not found' } }).end()
    return res.json({ payload: product }).end()
  })

  // get basket for session
  app.get('/api/basket', (req, res) => {
    if (basket.length > 4) return res.status(500).json({ error: { message: 'Server error' } }).end()
  ...
  app.post('/api/basket/product/:id', (req, res) => {
    ...
    const product = _.find(products, { id: id })
    if (!product) return res.status(404).json({ error: { message: 'Product not found' } }).end()
    if (!product.stocked) return res.status(409).json({ error: { message: 'Product not in stock' } }).end()
    ...
    res.status(201).json({ payload: basket }).end()
  })
  ...
  app.delete('/api/basket/product/:id', (req, res) => {
    ...
    const index = _.findIndex(basket, { id: id })
    const product = _.find(products, { id: id })
    if (!product || index === -1) return res.status(404).json({ error: { message: 'Product not found' } }).end()
  ...
  app.all('/api/*', (req, res) => res.status(404).json({ error: { message: 'Not found' } }).end())
}
